...
|------|----------|-----------|--------------|
| **Local / Stdio** | Claude Desktop on your laptop | stdin/stdout + background OAuth on `:3000` | Single user, single HaloPSA tenant |
| **Docker** | Self-hosted, single host | Streamable HTTP on `:3000` | DCR optional; bind to localhost or trusted LAN |
| **AKS / Kubernetes** | Internet-exposed multi-user MCP server (Claude.ai) | Streamable HTTP behind ingress + TLS | DCR gated, NetworkPolicy, HSTS, read-only rootfs |

Use the same image/binary for all three; only environment variables and surrounding infra change.

## 1. Local Dev (Claude Desktop / Stdio)
...
3. Configure Claude Desktop (`%APPDATA%\Claude\claude_desktop_config.json`):
   ...
   > Claude Desktop's `env` block does not pass through `wsl.exe`. Put env vars in `.env`.
...
4. Restart Claude Desktop. The first call prints a login URL; open it in a browser.
...
The MCP server ships with an offline dump of the HaloPSA reporting database schema in `schema/catalog.json` (845 tables, 15 domains). These tools let the LLM browse it cheaply when planning SQL — drill domains → tables → columns rather than dumping 3 MB into context.
